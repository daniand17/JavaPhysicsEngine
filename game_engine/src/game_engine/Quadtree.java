...
public class Quadtree {
	...
	/**
	 * Determines which node the object belongs to. -1 means object cannot completely fit within a
	...
	private int getIndex( GameObject ent ) {
		//TODO eventually use collider.getPositionInWorldSpace() when it is implemented
		Vector2 objLocation = ent.collider.getPosition();
		int index = -1 ;
		double verticalMidpoint = bounds.getX() + ( bounds.getWidth() / 2 ) ;
		double horizontalMidpoint = bounds.getY() + ( bounds.getHeight() / 2 ) ;
		// Object can completely fit within the top quadrants
		//TODO not sure if this is completely right with the getBounds().height...will only know once we are doing collisions
		boolean topQuadrant = ( objLocation.y < horizontalMidpoint && objLocation.y
				+ ent.collider.getBounds().height < horizontalMidpoint ) ;
		// Object can completely fit within the bottom quadrants
		boolean bottomQuadrant = ( objLocation.y > horizontalMidpoint ) ;
		// Object can completely fit within the left quadrants
		//TODO not sure if this is completely right with the getBounds().width...will only know once we are doing collisions
		if ( objLocation.x < verticalMidpoint && objLocation.x + ent.collider.getBounds().width < verticalMidpoint ) {
			if ( topQuadrant ) {
				index = 1 ;
